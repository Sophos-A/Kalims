...
class TriageInput(BaseModel):
    ...
    patientCategory: Optional[str] = Field(None, description="Patient category (new, follow-up, post-op, referral)")
    vulnerabilityFactors: Optional[List[str]] = Field(None, description="List of vulnerability factors")
...
async def run_triage(
    ...
    try:
        ...
        system_prompt = """
You are a neurosurgical triage AI assistant for an outpatient department. Analyze the patient's symptoms, category, and vulnerabilities to determine clinical urgency and appropriate queue prioritization.

Respond ONLY in valid JSON with these fields:
{
  "severityScore": float (0.0 to 1.0, where 1.0 is critical),
  "recommendedAction": string (e.g., "Immediate attention", "See within 30 mins", "Routine care"),
  "criticalFlags": array of strings (e.g., ["Severe headache", "Neurological deficit"]),
  "patientCategory": string ("new", "follow-up", "post-op", or "referral"),
  "vulnerabilityFactors": array of strings (e.g., ["elderly", "wheelchair"])  
}

Rules for Neurosurgical OPD Triage:
- Prioritize patients with acute neurological symptoms (headache, weakness, vision changes)
- Consider patient category (new patients may need more time, post-op may be urgent)
- Account for vulnerability factors (elderly, wheelchair-bound, urgent referrals)
- Be conservative: when in doubt, err on the side of urgency
- Use only the information provided
- Do not ask for more data — make best assessment
"""

        # Include patient category and vulnerability factors in the prompt
        patient_category = f"Patient Category: {data.patientCategory}" if data.patientCategory else "Patient Category: Not provided"
        vulnerability_factors = f"Vulnerability Factors: {', '.join(data.vulnerabilityFactors)}" if data.vulnerabilityFactors else "Vulnerability Factors: None"
        
        user_prompt = f"Symptoms: {data.symptoms}. Vitals: {data.vitals or 'Not provided'}. {patient_category}. {vulnerability_factors}."
